...
import (
	"azuki774/shorten/internal/factory"
	"context"
	"fmt"
...
func start() (err error) {
	...
	defer db.CloseDB()
	kg := factory.NewSourceGenerator()
	ap := factory.NewRegistService(l, db, kg)
	srv := factory.NewRegister(l, ap)
...
func init() {
	...
	startCmd.Flags().StringVar(&startOpt.DBInfo.Host, "db-host", "shorten-db", "DB Host")
	startCmd.Flags().StringVar(&startOpt.DBInfo.Port, "db-port", "3306", "DB Port")
	startCmd.Flags().StringVar(&startOpt.DBInfo.Name, "db-name", "shorten", "DB Name")
